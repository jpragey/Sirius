...
public class LambdaClosure {
	...
	private List<ClosureEntry> closureEntries;

	public LambdaClosure(List<ClosureEntry> closureEntries) {
		super();
	...
	public List<ClosureEntry> getClosureEntries() {
		return closureEntries;
	}
	
	public LambdaClosure appendEntry(ClosureEntry entry) {
		List<ClosureEntry> newEntries = Stream.concat(closureEntries.stream(), Stream.of(entry)) .collect(Collectors.toList());
	...
	public LambdaClosure appendEntry(AstType type, AstToken name, Optional<AstExpression> initExpr) {
		return appendEntry(new ClosureEntry(type, name, initExpr));
	}
	public LambdaClosure appendEntry(AstType type, AstToken name) {
		return appendEntry(new ClosureEntry(type, name));
	}
